...
const regex = /const getBankerBookmarkletCode = \(isDesktop: boolean = false\) => \{([\s\S]*?)const handleSubmitToHKJC =/;
const match = content.match(regex);

if (match) {
  const newBody = `const getBankerBookmarkletCode = (isDesktop: boolean = false) => {
  ...
  const getBookmarkletCode = (isDesktop: boolean = false) => {
    const defaultBets = generatedBets.filter(b => !b.isBankerLegs || (b.bankersCount || 0) === 0);
  ...
  const newContent = content.substring(0, match.index) + newBody + content.substring(match.index + match[0].length - 27);
