...
export default function App() {
  ...
  const [preferredOddCount, setPreferredOddCount] = useState<number | null>(
    null,
  );
  const [preferredEvenCount, setPreferredEvenCount] = useState<number | null>(
    null,
  );
  ...
  const [complexExcludeRanges, setComplexExcludeRanges] = useState<
    { start: number; end: number }[]
  >([{ start: 1, end: 5 }]);
  const [complexIncludeRanges, setComplexIncludeRanges] = useState<
    { start: number; end: number }[]
  >([{ start: 6, end: 10 }]);
  ...
  const [excludeUnseenIncludeSpecial, setExcludeUnseenIncludeSpecial] =
    useState(false);
  ...
  const [generatedBets, setGeneratedBetsInternal] = useState<
    import("@/lib/marksix").GeneratedBet[]
  >([]);
  const setGeneratedBets = (
    val:
      | import("@/lib/marksix").GeneratedBet[]
      | ((
          prev: import("@/lib/marksix").GeneratedBet[],
        ) => import("@/lib/marksix").GeneratedBet[]),
  ) => {
    setGeneratedBetsInternal((prev) => {
      ...
      for (const bet of raw) {
        ...
        if (bet.isBankerLegs && bet.bankersCount) {
          ...
          const bk = [...bet.numbers.slice(0, bet.bankersCount)]
            .sort((a: any, b: any) => a - b)
            .join(",");
          const lg = [...bet.numbers.slice(bet.bankersCount)]
            .sort((a: any, b: any) => a - b)
            .join(",");
  ...
  const [undoStack, setUndoStack] = useState<
    { index: number; bet: import("@/lib/marksix").GeneratedBet }[]
  >([]);
  const [isGenerating, setIsGenerating] = useState(false);
  const [analysisDrawIndex, setAnalysisDrawIndex] = useState<number | null>(
    null,
  );
  ...
  const [checkResults, setCheckResults] = useState<
    { matches: number[]; specialMatch: boolean }[] | null
  >(null);
  ...
  const getRawDrawNumbers = (
    draw: number[] | { numbers: number[]; date: string },
  ): number[] => {
  ...
  const [errorModal, setErrorModal] = useState<{
    message: string;
    partialBets?: import("@/lib/marksix").GeneratedBet[];
  } | null>(null);
  const [viewingBetExpl, setViewingBetExpl] = useState<{
    index: number;
    bet: import("@/lib/marksix").GeneratedBet;
  } | null>(null);
  const [selectedBetModal, setSelectedBetModal] = useState<{
    bet: import("@/lib/marksix").GeneratedBet;
    index: number;
  } | null>(null);
  ...
  const handleAddCoverUnselectedBet = () => {
    ...
    const allGeneratedNumbers = [...generatedBets]
      .map((b: any) => b.numbers)
      .flat();
    const usedNums = new Set<number>(allGeneratedNumbers);
    const unselected = Array.from({ length: 49 }, (_, i) => i + 1).filter(
      (n) => !usedNums.has(n),
    );
    ...
    const individualBudget = Math.max(
      10,
      Math.floor(coverBudget / betsCountToGenerate),
    );
    const newBets: any[] = [];
    ...
    for (let i = 0; i < betsCountToGenerate; i++) {
      ...
      while (attempts < 200 && !success) {
        ...
        if (shiftedUnselected.length >= B) {
          const startIdx =
            attempts === 1 ? (i * B) % shiftedUnselected.length : 0;
          for (let j = 0; j < B; j++) {
        ...
        while (bankers.length < B) {
          let pool = Array.from({ length: 49 }, (_, k) => k + 1).filter(
            (n) => !bankers.includes(n),
          );
        ...
        // Legs pool
        let remainingUnselected = unselected.filter(
          (n) => !bankers.includes(n),
        );
        ...
        while (
          getCombinationsCount(L + 1, requiredLegsCount) * 10 <=
            individualBudget &&
          L + 1 <= 49 - B
        ) {
          L++;
        ...
        if (remainingPool.length >= L) {
          ...
          let extrasPool = availableExtras.filter(
            (n) => !bankers.includes(n) && !legs.includes(n),
          );
  ...
  const handleGenerate = () => {
    ...
    try {
      ...
      const bets = generateBets({
        ...
        recentMode: enableRecent
          ? (recentMode as "exclude" | "include")
          : "none",
        recentCount,
        ...
        excludeUnseenInRecent:
          (enableRecent || enableComplexRecent) && enableExcludeUnseen
            ? excludeUnseenCount
            : undefined,
  ...
  const handleAIGenerate = () => {
    ...
    try {
      const rawRecentDraws = liveResults
        .slice(0, aiAnalysisDraws)
        .map(getRawDrawNumbers);
      ...
      const totalComboBets = Math.max(
        1,
        Math.round(aiBetCount * comboPercentage),
      );
      ...
      while (cbRemaining > 0) {
        if (
          allocatedCombos[0] === counts[0] &&
          allocatedCombos[1] === counts[1] &&
          allocatedCombos[2] === counts[2]
        ) {
          break;
        }
      ...
      if (aiBankerMode) {
        ...
        const aiRanking = Array.from(
          new Set(rawBets.flatMap((b) => b.numbers)),
        );
        ...
        const individualBudget = Math.max(
          10,
          Math.floor(aiBankerBudget / betsToGenerate),
        );
        ...
        for (let i = 0; i < betsToGenerate; i++) {
          ...
          while (attempts < 200 && !success) {
            ...
            const offset =
              (i * B + (attempts > 1 ? Math.floor(Math.random() * 49) : 0)) %
              aiRanking.length;
            for (let j = 0; j < B; j++) {
            ...
            let legs: number[] = [];
            const legsOffset =
              (i * L + (attempts > 1 ? Math.floor(Math.random() * 49) : 0)) %
              legsPool.length;
            for (let j = 0; j < L; j++) {
            ...
            if (!usedKeySet.has(key)) {
              ...
              const actualCost =
                getCombinationsCount(legs.length, requiredLegsCount) * 10;
              allBets.push({
                numbers: [...bankers, ...legs],
                explanations: [
        ...
        for (let s = 0; s < 3; s++) {
          ...
          if (s === 0) {
            ...
            stratName =
              "激進組 (冷門捕捉) - 應用拉伸間距分析，捕捉長期遺漏的「壁花」號碼";
            complexStrategy = {
              enabled: true,
              excludeRanges: [{ start: 1, end: 5 }],
              includeRanges: [],
            };
          ...
          const injectDynamicExplanations = (bet: any, typeName: string) => {
            let expls =
              bet.explanations && bet.explanations.length > 0
                ? [...bet.explanations]
                : [];
            ...
            if (sum < 110 || sum > 190) {
              expls.push(
                `總和值分佈：此注總和為 ${sum}，屬於極端分佈。AI 已觸發 30% 多樣性放行機制，保留此邊緣數據以應對「黑天鵝」隨機事件。`,
              );
            } else {
              expls.push(
                `總和值分佈：此注總和為 ${sum}，處於正態分佈核心 110~190 區間內，符合大數法則的常態預期。`,
              );
            }

            if (currentAiStrategy === "hot") {
              expls.push(
                `飽和衰減制動：已自動降低近期出現超過4次之過熱號碼的比重，防止誤墮「熱號陷阱」。`,
              );
            } else if (currentAiStrategy === "cold") {
              expls.push(
                `FFG 遺漏分析：已對遺漏超過 10 期、20 期以上的號碼進行指數級權重提升，精準捕捉回歸均值的「壁花」。`,
              );
            } else if (currentAiStrategy === "balanced") {
              expls.push(
                `動態平衡演算：同時應用 FFG 遺漏捕捉與飽和衰減制動，維持冷熱號碼的正態平衡。`,
              );
            }
            ...
            bet.numbers.forEach(
              (n: number) =>
                rbCounts[getBallColor(n) as keyof typeof rbCounts]++,
            );
            let colorParts = [];
          ...
          if (normalBetCountForS > 0) {
            const normalBets = generateBets({
              ...
              injectDynamicExplanations(
                bet,
                `此注採用純機率分佈進行獨立選號，透過先進隨機洗牌演算法排除人為偏差，確保每個數字在統計學上具有完全平等的出現機率。`,
              ),
            );
          ...
          if (comboBetCountForS > 0) {
            const comboBets = generateBets({
              ...
              injectDynamicExplanations(
                bet,
                `使用「2合策略」${willUse3Combos ? "及「3合策略」" : ""}，自動尋找最高勝率的同伴號碼組合。`,
              ),
            );
  ...
  const handleManualCheck = () => {
    try {
      ...
      checkManualInput.split("\n").forEach((line) => {
        ...
        if (bankerMatch) {
          const bankers = Array.from(
            new Set(
              (bankerMatch[1].match(/\d+/g) || [])
                .map((n) => parseInt(n, 10))
                .filter((n) => n >= 1 && n <= 49),
            ),
          );
          ...
          if (
            bankers.length > 0 &&
            legs.length > 0 &&
            bankers.length + legs.length >= 6
          ) {
            const requiredLegs = 6 - bankers.length;
            if (requiredLegs > 0) {
              getCombos(legs, requiredLegs).forEach((c) =>
                parsedBets.push([...bankers, ...c].sort((a, b) => a - b)),
              );
        ...
        const cleanedLine = trimmed.replace(
          /^(?:\d+[\.\)\]]\s*|(?:注|bet|第)\s*\d+\s*(?:注)?\s*[:：\.]?\s*)/gi,
          "",
        );
        const nums = (cleanedLine.match(/\d+/g) || [])
          .map((n) => parseInt(n, 10))
  ...
  const processScreenshotForCheck = async (file: File) => {
    ...
    toast.loading(
      <div className="text-center w-full font-bold text-[16px] text-zinc-700">
        正在解析圖片中的號碼...
      </div>,
      { id: "check-screenshot" },
    );
    ...
    try {
      ...
      if (qrData) {
        ...
        try {
          ...
          if (
            ...
            parsed.every((val) => Array.isArray(val) && val.length === 6)
          ) {
            validBets = parsed;
          }
        } catch (e) {}
        ...
        if (validBets.length > 0) {
          ...
          toast.success(
            <div className="text-center flex-1 font-bold text-xl">
              成功讀取號碼！
            </div>,
            { id: "check-screenshot" },
          );
      ...
      const base64DataReplaced = base64data.includes(",")
        ? base64data.split(",")[1]
        : base64data;
      ...
      if (Array.isArray(parsed)) {
        const validBets = parsed.filter(
          (b: any) =>
            Array.isArray(b) &&
            b.length === 6 &&
            b.every((n: any) => typeof n === "number" && n >= 1 && n <= 49),
        );
        if (validBets.length > 0) {
          handlePerformCheck(validBets);
          toast.success(
            <div className="text-center flex-1 font-bold">成功解析號碼！</div>,
            { id: "check-screenshot" },
          );
        } else {
  ...
  const handleCaptureScreenshot = async () => {
    ...
    try {
      ...
      console.error(e);
      toast.error(`截圖失敗: ${e?.message || "未知錯誤"}`, {
        id: "capture-toast",
      });
    }
  };
  ...
  const handleFloatingWindow = async () => {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        ${generatedBets
          .map((bet, i) => {
            if (bet.isBankerLegs && bet.bankersCount) {
              const htmlBankers = bet.numbers
                .slice(0, bet.bankersCount)
                .map((n) => {
                ...
                .join("");
              const htmlLegs = bet.numbers
                .slice(bet.bankersCount)
                .map((n) => {
                  const color = getBallColor(n);
                  const colorClass =
                    color === "red"
    ...
    const popup = window.open(
      "",
      "HKJC_Helper",
      "width=280,height=400,left=100,top=100",
    );
    if (popup) {
      popup.document.open();
  ...
  return (
    <div className="min-h-screen text-zinc-900 pb-20 font-sans selection:bg-[#FF4D4D] selection:text-white">
        ...
        <div className="max-w-[1600px] mx-auto px-2 sm:px-4 h-12 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3 shrink-0 sm:shrink sm:flex-1 min-w-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#FF4D4D] border-[3px] sm:border-4 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center font-black transform -rotate-6 shrink-0">
              <span
                className="text-[#FFE867] text-xl sm:text-[26px] leading-none mb-[1px]"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2 min-w-0 pb-0.5 sm:flex-1">
              ...
              <div className="hidden lg:flex flex-1 justify-between gap-x-3 gap-y-10 flex-wrap overflow-hidden px-4 pl-8 self-center max-h-[44px] pt-0.5">
                {Array.from({ length: 15 }).map((_, i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-full bg-[#FF4D4D] border-4 border-black flex items-center justify-center font-black transform -rotate-6 shrink-0 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                  >
                    <span
                      className="text-[#FFE867] text-[26px] leading-none mb-[1px]"
                      style={{ WebkitTextStroke: "1.5px black" }}
                    >
                      $
                    </span>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (isAndroid) {
                  window.open(
                    "intent://bet.hkjc.com/ch/marksix/home#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=https%3A%2F%2Fbet.hkjc.com%2Fch%2Fmarksix%2Fhome;end",
                    "_top",
                  );
                } else if (isIOS) {
                  window.open(
                    "googlechrome://bet.hkjc.com/ch/marksix/home",
                    "_top",
                  );
                  setTimeout(() => {
                    window.open(
                      "https://bet.hkjc.com/ch/marksix/home",
                      "_blank",
                    );
                  }, 1000);
                } else {
      ...
      <main className="max-w-[1600px] mx-auto px-0 sm:px-4 pt-0 pb-2 sm:pt-0 sm:pb-2 bg-black">
        <div
          className={`grid grid-cols-1 ${generatedBets.length === 0 ? "lg:grid-cols-12" : ""} gap-1.5 sm:gap-4`}
        >
                ...
                <CardContent className="space-y-1.5 px-3 pb-3 pt-2 sm:space-y-2 sm:px-4 sm:pb-4 sm:pt-3">
                    ...
                    {ranges.map((range, index) => (
                      <div key={index} className="space-y-1">
                        <div className="flex justify-end items-center">
                          {ranges.length > 1 && (
                            <span className="text-sm font-bold text-zinc-500 mr-auto">
                              範圍 {index + 1}
                            </span>
                          )}
                  ...
                  <div className="space-y-0.5">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button
                        onClick={() => {
                          setOddEven("all");
                          setPreferredOddCount(3);
                          setPreferredEvenCount(3);
                        }}
                      ...
                      <button
                        onClick={() => {
                          setOddEven("all");
                          setPreferredOddCount(4);
                          setPreferredEvenCount(2);
                        }}
                      ...
                      <button
                        onClick={() => {
                          setOddEven("all");
                          setPreferredOddCount(2);
                          setPreferredEvenCount(4);
                        }}
                      ...
                      <button
                        onClick={() => {
                          setOddEven("all");
                          setPreferredOddCount(5);
                          setPreferredEvenCount(1);
                        }}
                      ...
                      <button
                        onClick={() => {
                          setOddEven("all");
                          setPreferredOddCount(1);
                          setPreferredEvenCount(5);
                        }}
                    ...
                    {colors.length === 2 && (
                      <div className="mt-4 pt-4 border-t-2 border-black border-dashed flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          <span
                            className={`flex-none flex items-center justify-center font-black aspect-square w-8 h-8 text-sm sm:text-base rounded-full border-[2.5px] border-black shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] ${colors[0] === "red" ? "bg-[#FF9999] text-black" : colors[0] === "blue" ? "bg-[#99CCFF] text-black" : "bg-[#99FF99] text-black"}`}
                          >
                            {colors[0] === "red"
                              ? "紅"
                              : colors[0] === "blue"
                          ...
                          />
                          <span
                            className={`flex-none flex items-center justify-center font-black aspect-square w-8 h-8 text-sm sm:text-base rounded-full border-[2.5px] border-black shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] ${colors[1] === "red" ? "bg-[#FF9999] text-black" : colors[1] === "blue" ? "bg-[#99CCFF] text-black" : "bg-[#99FF99] text-black"}`}
                          >
                            {colors[1] === "red"
                              ? "紅"
                              : colors[1] === "blue"
                  ...
                  <div className="space-y-0.5">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        onClick={() =>
                          setNoConsecutivePairs(!noConsecutivePairs)
                        }
                      ...
                      <button
                        onClick={() =>
                          setNoConsecutiveTriplets(!noConsecutiveTriplets)
                        }
                  ...
                  <div className="space-y-1.5 pt-1 border-t-[3px] border-black border-dashed">
                    <div className="flex flex-col gap-2">
                      <Label className="text-base font-bold flex items-center gap-2">
                        選擇你的幸運號碼
                        <span className="text-[11px] bg-[#FFE867] px-1.5 py-[1px] border border-black rounded shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">
                          選填
                        </span>
                      </Label>
                      ...
                      <div className="flex gap-2">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {luckyNumbers.map((num) => {
                                ...
                                return (
                                  <div
                                    key={num}
                                    className={`w-8 h-8 rounded-full border-2 border-black flex items-center justify-center font-black text-sm text-black ${bgColor}`}
                                  >
                                    {num}
                                  </div>
                                );
                        ...
                        <Dialog
                          open={isLuckyDialogOpen}
                          onOpenChange={setIsLuckyDialogOpen}
                        >
                              ...
                              {MARK_SIX_NUMBERS.map((num) => {
                                ...
                                return (
                                  <button
                                    ...
                                    ${
                                      isSelected
                                        ? getBallColor(num) === "red"
                                          ? "bg-[#FF9999] text-black border-zinc-900 shadow-none translate-y-[1px] translate-x-[1px]"
                                          : getBallColor(num) === "blue"
                                            ? "bg-[#99CCFF] text-black border-zinc-900 shadow-none translate-y-[1px] translate-x-[1px]"
                                            : "bg-[#99FF99] text-black border-zinc-900 shadow-none translate-y-[1px] translate-x-[1px]"
                                        : isDisabled
                                          ? "bg-zinc-100 text-zinc-400 border-zinc-300 shadow-none cursor-not-allowed opacity-50"
                                          : "bg-white text-black hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none"
                                    }
                                  `}
                                  >
                                    {num}
                        ...
                        <div className="space-y-1 mt-0.5 flex flex-col gap-0.5 mb-2">
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="flex items-center gap-1.5 bg-white border-[3px] border-black py-0.5 px-1.5 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] w-fit cursor-pointer hover:bg-zinc-50 relative active:translate-y-[2px] active:translate-x-[2px] active:shadow-none transition-all">
                              ...
                              />
                              <span className="text-[11px] sm:text-xs whitespace-nowrap select-none">
                                連特別號碼一齊考慮
                              </span>
                            </label>
                      ...
                      {enableComplexRecent &&
                        (() => {
                          const hasOverlap = complexExcludeRanges.some((ex) =>
                            complexIncludeRanges.some(
                              (inc) =>
                                Math.max(ex.start, inc.start) <=
                                Math.min(ex.end, inc.end),
                            ),
                          );
                            ...
                            <div className="space-y-4 mt-1 p-3 bg-zinc-50 border-[3px] border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-xl relative">
                              <div className="flex flex-col gap-2">
                                ...
                                {complexExcludeRanges.map((range, index) => (
                                  ...
                                  >
                                    <div className="flex justify-end items-center">
                                      {complexExcludeRanges.length > 1 && (
                                        <span className="text-xs font-bold text-zinc-500 mr-auto">
                                          範圍 {index + 1}
                                        </span>
                                      )}
                                  ...
                                  <div className="flex gap-1.5">
                                    {complexIncludeRanges.length < 3 && (
                                      <Button
                                        ...
                                        className="h-6 px-2 text-[11px] border-[2px] border-black font-bold shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-0.5 hover:translate-x-0.5 hover:shadow-[0px_0px_0px_0px_rgba(0,0,0,1)] transition-all bg-white"
                                        onClick={() =>
                                          setComplexIncludeRanges([
                                            ...complexIncludeRanges,
                                            { start: 1, end: 5 },
                                          ])
                                        }
                                      >
                              ...
                              <label className="flex items-center gap-1.5 bg-white border-[3px] border-black py-1 px-2 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] w-fit cursor-pointer hover:bg-zinc-50 relative active:translate-y-[2px] active:translate-x-[2px] active:shadow-none transition-all mt-2">
                                <input
                                  ...
                                  className="w-4 h-4 accent-[#3b82f6] cursor-pointer"
                                />
                                <span className="text-xs sm:text-sm whitespace-nowrap select-none">
                                  連特別號碼一齊考慮
                                </span>
                              </label>
                            </div>
                        ...
                        <div className="flex flex-col gap-1 mt-2 border-t-[2px] border-black border-dashed pt-2">
                          <label className="flex items-center gap-1.5 cursor-pointer">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) =>
                                setEnableExcludeUnseen(checked as boolean)
                              }
                              className="w-4 h-4 border-[3px] border-black rounded-sm data-[state=checked]:bg-[#FF4D4D] data-[state=checked]:text-white"
                            />
                            <span className="text-xs sm:text-sm whitespace-nowrap select-none flex items-center gap-1">
                              排除近期沒有出現過的所有號碼
                            </span>
                          </label>
                          {enableExcludeUnseen && (
                            <div className="flex flex-col gap-1.5 mb-1 w-full pl-4">
                              ...
                              <div className="flex items-center gap-1.5 mt-1.5 ml-1 mb-1.5 w-fit">
                                <label className="flex items-center gap-1.5 bg-white border-[3px] border-black py-0.5 px-1.5 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] cursor-pointer hover:bg-zinc-50 relative active:translate-y-[2px] active:translate-x-[2px] active:shadow-none transition-all">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setExcludeUnseenIncludeSpecial(
                                        e.target.checked,
                                      )
                                    }
                                    className="w-4 h-4 accent-[#3b82f6] cursor-pointer"
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setShowUnseenNumbers(!showUnseenNumbers)
                                }
                              >
                                {showUnseenNumbers
                                  ? "隱藏沒有出現過的號碼"
                              ...
                              {showUnseenNumbers &&
                                (() => {
                                  if (liveResults.length === 0)
                                    return (
                                      <div className="text-xs text-zinc-500 font-bold ml-1">
                                        載入中...
                                      </div>
                                    );
                                  const rawRecentDraws =
                                    liveResults.map(getRawDrawNumbers);
                                  const drawsToConsider = rawRecentDraws
                                    .slice(0, excludeUnseenCount)
                                    .map((draw) =>
                                      excludeUnseenIncludeSpecial
                                        ? draw
                                        : draw.slice(0, 6),
                                  ...
                                  const seenNumbers = new Set(
                                    drawsToConsider.flat(),
                                  );
                                  const unseenNumbers = MARK_SIX_NUMBERS.filter(
                                    (num) => !seenNumbers.has(num),
                                  );

                                  return (
                                    <div className="mt-1 mr-4 p-2.5 bg-white border-[3px] border-black rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                      <div className="text-xs font-black mb-2 text-zinc-700">
                                        此 {excludeUnseenCount}{" "}
                                        期沒有出現過的號碼 (共{" "}
                                        {unseenNumbers.length} 個):
                                      </div>
                                      <div className="flex flex-wrap gap-1.5">
                                        {unseenNumbers.map((n) => {
                                          const color = getBallColor(n);
                                          ...
                                          return (
                                            <span
                                              key={n}
                                              className={`flex items-center justify-center w-7 h-7 rounded-full border-[2px] font-black text-xs shadow-sm ${bgClass}`}
                                            >
                                              {n}
                                            </span>
                                          );
                        ...
                        <div className="flex flex-row items-center justify-between w-full">
                          <div className="flex flex-col gap-3 ml-1 shrink-0">
                            <label className="flex items-center gap-1.5 cursor-pointer">
                              ...
                              />
                              <span className="font-bold text-sm sm:text-[15px] whitespace-nowrap select-none">
                                採用「2合」策略
                              </span>
                            </label>
                            <label className="flex items-center gap-1.5 cursor-pointer">
                ...
                <CardFooter className="bg-zinc-100 border-t-[3px] border-black py-2 px-3 sm:py-3 sm:px-4 m-0 rounded-none w-full flex flex-col justify-center gap-3 mt-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full justify-center mt-2 flex-wrap">
                    <Button
                      ...
                      onClick={() =>
                        document
                          .getElementById("regenerate-api-upload")
                          ?.click()
                      }
                    >
                      上載系統截圖重新生成
                    ...
                    {generatedBets.length > 0 &&
                      !generatedBets.some(
                        (b: any) => b.isBankerLegs && (b.bankersCount || 0) > 0,
                      ) && (
                        <Button
                          variant="outline"
          ...
          <div
            ...
            className={
              generatedBets.length === 0
                ...
                : generatedBets.length > 5
                  ? "max-w-6xl mx-auto w-full space-y-2 sm:space-y-4"
                  : "max-w-2xl mx-auto w-full space-y-1 sm:space-y-2"
            }
          >
            {generatedBets.length > 0 ? (
              <div className="space-y-1 sm:space-y-2">
                <div className="flex flex-col gap-1 items-center justify-center w-full">
                  ...
                  {(() => {
                    ...
                    allGeneratedNumbers.forEach((n) =>
                      freqMap.set(n, (freqMap.get(n) || 0) + 1),
                    );

                    const recentDrawsToConsider = liveResults.slice(
                      0,
                      recentCount,
                    );
                    const historicalFreqMap = new Map<number, number>();
                    Array.from(freqMap.keys()).forEach((n) => {
                      historicalFreqMap.set(
                        n,
                        recentDrawsToConsider.filter((draw) =>
                          getRawDrawNumbers(draw).includes(n),
                        ).length,
                      );
                    });

                    // Sort primarily by historical frequency (descending), then generated frequency (descending), then by value (ascending)
                    const uniqueGeneratedNumbers = Array.from(
                      freqMap.keys(),
                    ).sort((a, b) => {
                      const histFreqDiff =
                        historicalFreqMap.get(b)! - historicalFreqMap.get(a)!;
                    ...
                    const expectedUnique =
                      49 * (1 - Math.pow(43 / 49, generatedBets.length)); // Expected unique numbers for purely random selection
                    // Recommend Banker only if unique numbers are very low (e.g. <= 16) so combinations don't explode
                    const isHighlyRepeated =
                      generatedBets.length >= 3 &&
                      uniqueGeneratedNumbers.length > 6 &&
                      uniqueGeneratedNumbers.length <= 16;
                    ...
                    const handleToggleBanker = (num: number) => {
                      setBankers((prev) => {
                        if (prev.includes(num)) {
                          ...
                          setExcludedLegs((prevExcluded) =>
                            prevExcluded.filter((n) => n !== num),
                          );
                    ...
                    const activeLegs = uniqueGeneratedNumbers.filter(
                      (n) => !bankers.includes(n) && !excludedLegs.includes(n),
                    );
                    const totalBetsWithBankers = getCombinationsCount(
                      activeLegs.length,
                      6 - bankers.length,
                    );
                    ...
                    return (
                      <div className="bg-[#ffedd5] max-w-3xl mx-auto border-4 border-black rounded-2xl p-3 sm:p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-2 relative mt-1 sm:mt-2 mb-2 w-full">
                        ...
                        <p className="text-xs sm:text-sm font-bold text-zinc-700">
                          這 {generatedBets.length} 注號碼僅由{" "}
                          {uniqueGeneratedNumbers.length} 個不同數字組成。挑選
                          1-5
                          個心水「膽」，不僅能覆蓋所有生成的數字，還能省下注本！(號碼已按歷史出現次數由高至低排列)
                        </p>
                        ...
                        <div className="bg-white border-[3px] border-black rounded-xl p-2 sm:p-3 mt-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] space-y-3">
                          <div className="space-y-1.5">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="font-bold text-[11px] sm:text-xs text-black border-2 border-black bg-[#FFD700] px-2 py-0.5 rounded-md inline-block w-fit">
                                第一步：選 1-5 個「膽」
                              </span>
                              <span className="text-[10px] sm:text-[11px] font-bold text-zinc-500">
                                球上方數字為近 {recentCount} 期出現次數
                              </span>
                            </div>
                            <div className="flex flex-wrap justify-center gap-2 sm:gap-3 items-end pt-0.5">
                              {uniqueGeneratedNumbers.map((num) => {
                                ...
                                const pastRecentFreq = liveResults
                                  .slice(0, recentCount)
                                  .filter((draw) =>
                                    getRawDrawNumbers(draw).includes(num),
                                  ).length;
                          ...
                          {bankers.length > 0 && (
                            <div className="pt-3 border-t-[3px] border-black border-dashed flex flex-col gap-2">
                              <div className="space-y-1.5">
                                <span className="font-bold text-[11px] sm:text-xs text-black border-2 border-black bg-[#dcfce7] px-2 py-0.5 rounded-md inline-block w-fit">
                                  第二步：點擊剔除不需要的配腳 (可省略)
                                </span>
                                <div className="flex flex-wrap justify-center gap-1 sm:gap-1.5 pt-0.5 p-2 bg-zinc-50 border-[3px] border-black rounded-lg">
                                  {uniqueGeneratedNumbers
                                    .filter((n) => !bankers.includes(n))
                                    .map((num) => {
                                      const isExcluded =
                                        excludedLegs.includes(num);
                                      const color = getBallColor(num);
                              ...
                              <div className="flex flex-col gap-1.5 mt-1">
                                <div className="text-sm sm:text-base font-bold text-black flex flex-wrap gap-x-3 gap-y-1 bg-zinc-50 border-2 border-black rounded-lg p-2.5">
                                  <span className="flex items-center gap-1.5">
                                    ...
                                    <span className="font-black text-[#FF4D4D]">
                                      {bankers
                                        .map((n) => n.toString())
                                        .join(", ")}
                                    </span>
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 items-center">
                    ...
                    <Button
                      ...
                      onClick={
                        isAiGenerated ? handleAIGenerate : handleGenerate
                      }
                    >
                    ...
                    <div className="flex w-full sm:w-auto gap-1.5 sm:gap-2">
                      ...
                      >
                        <div className="flex flex-col items-center leading-tight">
                          ...
                          <span className="text-[10px] sm:text-xs opacity-90 mt-0.5">
                            (內含 QR Code 方便再次對獎)
                          </span>
                        </div>
                      </Button>
                          ...
                          <div className="p-6 sm:p-8 overflow-y-auto w-full grow custom-scrollbar min-h-0">
                            <DialogHeader>
                              ...
                              <DialogDescription className="font-bold text-black/80 text-sm sm:text-base space-y-2 flex flex-col">
                                <span>
                                  由於瀏覽器安全限制，我們無法直接控制 HKJC
                                  網頁。請使用以下「自動點擊腳本」來代替手動按球。
                                </span>
                                <span className="text-[#3b82f6] bg-[#3b82f6]/10 p-2 rounded-lg border-2 border-[#3b82f6]/20">
                                  此教學專為電腦版瀏覽器設計。如需手機版，請關閉此視窗並點擊「自動點擊
                                  HKJC (手機版)」按鈕。
                                </span>
                              </DialogDescription>
                            </DialogHeader>
                            ...
                            <div className="w-full mt-4 space-y-4">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const script = decodeURIComponent(
                                      getBookmarkletCode(true).replace(
                                        "javascript:",
                                        "",
                                      ),
                                    );
                              ...
                              <div className="space-y-2">
                                ...
                                <ol className="list-decimal list-inside space-y-1.5 font-bold text-sm text-zinc-700">
                                  ...
                                  <li>
                                    按下鍵盤{" "}
                                    <kbd className="bg-zinc-200 px-1.5 py-0.5 rounded border-2 border-black text-black">
                                      F12
                                    </kbd>{" "}
                                    打開開發者工具。
                                  </li>
                                  <li>
                                    切換到 <strong>Console (控制台)</strong>{" "}
                                    標籤。
                                  </li>
                                  <li>
                                    貼上剛剛複製的腳本，然後按下{" "}
                              ...
                              <div className="pt-3 border-t-4 border-black border-dashed">
                                ...
                                <p className="text-[11px] font-bold text-[#FF4D4D] bg-[#FF4D4D]/10 p-1.5 rounded-md border border-[#FF4D4D]/20">
                                  ⚠️
                                  注意：書籤的號碼是固定的！每次重新生成號碼後，您必須刪除舊書籤，並「重新拖曳」一次新的按鈕到書籤列。
                                </p>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="p-6 sm:p-8 overflow-y-auto w-full grow custom-scrollbar min-h-0">
                              <DialogHeader>
                                ...
                                <DialogDescription className="text-black/80 text-sm sm:text-base space-y-2 flex flex-col">
                                  <span>
                                    請依據以下步驟，在手機瀏覽器 (Safari 或
                                    Chrome) 設定自動點擊「書籤腳本」。
                                    <br />
                                    設定完成後便可於 HKJC 投注頁面執行。
                                  </span>
                                </DialogDescription>
                              </DialogHeader>
                              ...
                              <div className="w-full mt-4 space-y-5">
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        const scriptContent =
                                          decodeURIComponent(
                                            getBookmarkletCode(false).replace(
                                              "javascript:",
                                              "",
                                            ),
                                          );
                                ...
                                <div className="space-y-2">
                                  ...
                                  <ol className="list-decimal list-inside space-y-2 text-sm text-zinc-700 bg-zinc-100 p-3 rounded-xl border-2 border-zinc-200">
                                    ...
                                    <li>
                                      將名稱改為貼上{" "}
                                      <kbd className="bg-[#FFE867] px-1.5 py-0.5 rounded border border-black text-black">
                                        自動按球
                                      </kbd>
                                      。
                                    </li>
                                    <li>
                                      將網址(URL) 全部清空，並 貼上腳本代碼。
                                      <br />
                                      <span className="text-xs text-red-500 block mt-1">
                                        (請檢查開頭是否包含 javascript:)
                                      </span>
                                    </li>
                                    <li>點擊 儲存。</li>
                                  </ol>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="text-sm text-zinc-700 bg-zinc-100 p-3 rounded-xl border-2 border-zinc-200 space-y-3">
                                    <p>
                                      ...
                                      <a
                                        ...
                                        onClick={(e) => {
                                          ...
                                          if (isAndroid) {
                                            window.open(
                                              "intent://bet.hkjc.com/ch/marksix/Single#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=https%3A%2F%2Fbet.hkjc.com%2Fch%2Fmarksix%2FSingle;end",
                                              "_top",
                                            );
                                          } else if (isIOS) {
                                            window.open(
                                              "googlechrome://bet.hkjc.com/ch/marksix/Single",
                                              "_top",
                                            );
                                            setTimeout(() => {
                                              window.open(
                                                "https://bet.hkjc.com/ch/marksix/Single",
                                                "_blank",
                                              );
                                            }, 1000);
                                          } else {
                                    ...
                                    <div className="border-l-4 border-[#3b82f6] pl-2 py-1 mb-2 mt-2">
                                      ...
                                      <br />
                                      點擊下方{" "}
                                      <kbd className="bg-white px-1.5 py-0.5 rounded border border-black text-black text-xs">
                                        📖 書籤
                                      </kbd>{" "}
                                      圖示，直接點擊{" "}
                                      <kbd className="bg-[#FFE867] px-1 py-0.5 rounded border border-black text-black text-xs">
                                        自動按球
                                      </kbd>
                                      。
                                    </div>
                                    ...
                                    <p className="text-[#FF4D4D] mt-3 tracking-wide">
                                      ▶
                                      程式即會幫您自動點擊號碼球和「加入注項」！
                                    </p>
                                  </div>
                              ...
                              <div className="w-full mt-4 space-y-5">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <ol className="list-decimal list-inside space-y-2 text-sm text-zinc-700 bg-zinc-100 p-3 rounded-xl border-2 border-zinc-200">
                                    ...
                                    <li>
                                      網址 (URL) 欄位貼上複製的：
                                      <kbd className="bg-[rgba(0,0,0,0.05)] px-1 py-0.5 rounded text-xs select-all">
                                        javascript:...
                                      </kbd>{" "}
                                      腳本代碼。
                                    </li>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="text-sm text-zinc-700 bg-zinc-100 p-3 rounded-xl border-2 border-zinc-200">
                                    <p className="leading-relaxed">
                                      前往{" "}
                                      <a
                                        href="https://bet.hkjc.com/ch/marksix/Banker"
                                        target="_blank"
                                        rel="noreferrer"
                                        className="text-blue-600 underline font-bold"
                                      >
                                    ...
                                    <p>
                                      ...
                                      <a
                                        ...
                                        onClick={(e) => {
                                          ...
                                          if (isAndroid) {
                                            window.open(
                                              "intent://bet.hkjc.com/ch/marksix/Banker#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=https%3A%2F%2Fbet.hkjc.com%2Fch%2Fmarksix%2FBanker;end",
                                              "_top",
                                            );
                                          } else if (isIOS) {
                                            window.open(
                                              "googlechrome://bet.hkjc.com/ch/marksix/Banker",
                                              "_top",
                                            );
                                            setTimeout(() => {
                                              window.open(
                                                "https://bet.hkjc.com/ch/marksix/Banker",
                                                "_blank",
                                              );
                                            }, 1000);
                                          } else {
                ...
                <div className="flex flex-wrap justify-center gap-2 sm:gap-4 w-full items-start">
                  {generatedBets.map((bet, index) => (
                    <div
                      key={index}
                      className={`flex flex-col items-center gap-1.5 ${bet.isBankerLegs ? "w-full max-w-[600px]" : "w-fit"}`}
                    >
                      <div
                        ...
                        onClick={() =>
                          isAiGenerated && setViewingBetExpl({ index, bet })
                        }
                      >
                        <div className="flex items-center justify-start gap-1 sm:gap-2 min-h-[42px] sm:min-h-[50px] pr-1 pointer-events-none w-full">
                          ...
                          <div
                            className={`py-1 ${bet.isBankerLegs ? "flex-1 grow flex flex-col items-center justify-center min-w-0" : "flex flex-wrap gap-0 sm:gap-0.5 items-center max-w-[calc(96vw-90px)] sm:max-w-[480px] md:max-w-[700px] lg:max-w-none shrink-0"}`}
                          >
                            {(() => {
                              ...
                              if (bet.isBankerLegs && bet.bankersCount) {
                                const legsCount =
                                  bet.numbers.length - bet.bankersCount;
                                  ...
                                  <div className="flex flex-col gap-1.5 items-center w-full my-1">
                                    <div className="relative">
                                      <div className="flex flex-wrap justify-center items-center gap-1 sm:gap-1.5 bg-yellow-100/50 px-2 sm:px-3 py-0.5 rounded-[18px] border-2 border-black/10">
                                        {bet.numbers
                                          .slice(0, bet.bankersCount)
                                          .map((num, i) => renderBall(num, i))}
                                      </div>
                                      <div className="absolute -bottom-2 -right-1.5 flex items-center justify-center w-[22px] h-[22px] sm:w-[26px] sm:h-[26px] rounded-full bg-black text-[#FFE867] font-black text-[10px] sm:text-[12px] border-[2px] border-white shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] z-10 transform rotate-6">
                                        拖
                                      </div>
                                    </div>
                                    <div
                  ...
                  <div className="mt-8 border-t-4 border-dashed border-black pt-6">
                    ...
                    <div className="flex flex-wrap justify-center gap-2 sm:gap-4 w-full items-start">
                      {specialCoverBets.map((bet, index) => (
                        <div
                          key={index}
                          className="flex flex-col items-center gap-1.5 w-fit"
                        >
                          <div
                            ...
                            onClick={() =>
                              setViewingBetExpl({
                                index: generatedBets.length + index,
                                bet,
                              })
                            }
                          >
                ...
                <div className="mt-4 sm:mt-6 bg-[#ffedd5] border-[3px] sm:border-4 border-black rounded-2xl p-3 sm:p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] max-w-2xl mx-auto w-full text-center">
                  <h3 className="font-black text-lg mb-1 flex items-center justify-center gap-1.5 min-w-0">
                  ...
                  <div className="text-sm font-bold text-zinc-700 flex flex-wrap gap-2 justify-center mt-2">
                    {isAiGenerated ? (
                      <>
                        ...
                        <span className="w-full text-xs text-zinc-500 mt-2">
                          💡
                          點擊上方任何一注號碼，可即時查看專屬的大數據選號說明。
                        </span>
                      </>
                    ) : generatedBets.length > 0 &&
                      generatedBets[0].id?.startsWith("unselected-cover-") ? (
                      <div className="w-full text-left bg-white border-2 border-black rounded-lg p-3 sm:p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] mt-2 mb-3">
                        ...
                        <ul className="list-disc pl-5 sm:pl-6 space-y-1.5 text-sm sm:text-[15px] font-bold text-zinc-700">
                          <li>
                            此組合專為「全包剩餘號碼」策略所產生，此為獨立生成的膽拖/單式組合，以上記錄與當初自定的生成條件及其他篩選設定無關。
                          </li>
                        </ul>
                      </div>
                    ) : (
                        ...
                        <ul className="list-disc pl-5 sm:pl-6 space-y-1.5 text-sm sm:text-[15px] font-bold text-zinc-700">
                          ...
                          {(noConsecutivePairs || noConsecutiveTriplets) && (
                            <li>
                              連號限制:{" "}
                              {[
                                noConsecutivePairs && "不要連2號",
                                noConsecutiveTriplets && "不要連3號",
                              ]
                                .filter(Boolean)
                                .join("、")}
                            </li>
                          ...
                          {(preferredOddCount !== null ||
                            ...
                            <li>
                              單雙配置:{" "}
                              {oddEven === "all" && preferredOddCount === null
                                ? "無限制"
                                : oddEven === "odd"
                                  ? "全單"
                                  : oddEven === "even"
                                    ? "全雙"
                                    : `特定比例: ${preferredOddCount}單 ${preferredEvenCount}雙`}
                            </li>
                          )}
                          {colors.length < 3 && (
                            <li>
                              波色配置:{" "}
                              {colors.length === 1
                                ? `全${colors[0] === "red" ? "紅" : colors[0] === "blue" ? "藍" : "綠"}波`
                                : `特定波色比例 (${colors[0] === "red" ? "紅" : colors[0] === "blue" ? "藍" : "綠"} ${6 - (colorRatioOption || 3)} : ${colorRatioOption || 3} ${colors[1] === "red" ? "紅" : colors[1] === "blue" ? "藍" : "綠"})`}
                            </li>
                          )}
                          {(use2Combos || use3Combos) && (
                            <li>
                              大數據策略: {use2Combos ? "2合策略 " : ""}
                              {use3Combos ? "3合策略" : ""}
                            </li>
                          )}
                          {enableRecent && recentMode === "include" && (
                            <li>近期名單過濾: 只買近 {recentCount} 期號碼</li>
                          )}
                          {enableRecent && recentMode === "exclude" && (
                            <li>近期名單過濾: 排除近 {recentCount} 期號碼</li>
                          )}
                          {enableExcludeUnseen && (
                            <li>
                              排除未開出過濾: 排除近 {excludeUnseenCount}{" "}
                              期內完全未開出的冷門號碼
                            </li>
                          )}
                          {enableComplexRecent && (
              ...
              <div
                className="h-full min-h-[500px] flex flex-col items-center bg-orange-400 border-[3px] sm:border-4 border-black rounded-2xl sm:rounded-3xl p-2 sm:p-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] sm:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]"
                style={{ paddingTop: "3px" }}
              >
                <div
                  className="flex flex-col items-center mb-3 sm:mb-4 pt-1 sm:pt-2 text-center gap-3 w-full"
                  style={{
                    paddingTop: "-13px",
                    paddingBottom: "-15px",
                    marginBottom: "11px",
                ...
                >
                  {nextDrawInfo && (
                    <div
                      className="bg-[#FFD700] border-[3px] border-black px-6 py-2.5 w-max max-w-full rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center relative mt-3 mb-1"
                      style={{ borderStyle: "groove", paddingBottom: "7px" }}
                    >
                      <span
                        className="text-xs sm:text-sm font-black px-3 py-0.5 rounded-full border-[2px] border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] absolute -top-3.5 sm:-top-4 left-1/2 -translate-x-1/2 whitespace-nowrap tracking-widest"
                        style={{ backgroundColor: "#f060ff", fontSize: "19px" }}
                      >
                        下一期預計頭獎
                      </span>
                      <div className="flex items-center gap-2 sm:gap-3 mt-2 sm:mt-1 font-black">
                        <span
                          className="text-[22px] sm:text-2xl drop-shadow-[1.5px_1.5px_0px_rgba(0,0,0,1)] px-1"
                          style={{
                            color: "#000000",
                            fontWeight: "bold",
                            fontFamily: "Verdana",
                        ...
                        <span className="text-black/30 font-black text-lg">
                          |
                        </span>
                        <span
                          className="text-xl sm:text-2xl drop-shadow-[1px_1px_0px_rgba(255,255,255,0.8)] px-1"
                          style={{
                            fontFamily: "Verdana",
                            paddingLeft: "8px",
                            paddingTop: "4px",
                  ...
                  <span
                    className="inline-block font-black text-lg sm:text-xl text-black bg-[#FFD700] px-3 py-1 border-[3px] border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] mt-2"
                    style={{
                      paddingTop: "1px",
                      paddingRight: "12px",
                      paddingBottom: "1px",
                ...
                <div
                  className="w-full flex-1 overflow-y-auto pr-1 space-y-2"
                ...
                >
                  {liveResultsLoading ? (
                    ...
                    displayPastResults.map((drawObj, index) => {
                      ...
                      const displayTitle =
                        index === 0 ? "最近一期" : `前 ${index} 期`;

                      return (
                        <div
                          key={index}
                          className="flex flex-col sm:flex-row items-center justify-center w-fit mx-auto bg-zinc-50 border-[3px] sm:border-[4px] border-black rounded-lg sm:rounded-xl py-1 px-1 sm:py-1.5 sm:px-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] sm:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] gap-1 sm:gap-2 mb-1.5 min-w-[280px]"
                        >
                          ...
                          <div className="flex gap-0 sm:gap-0.5 flex-nowrap items-center shrink-0 pr-0.5 pb-0.5 sm:pb-0">
                            {numbers.map((num, numIdx) => {
                              ...
                              return (
                                ...
                                >
                                  {isSpecial && (
                                    <span className="font-black text-sm sm:text-xl text-zinc-400 px-0.5 sm:px-1">
                                      +
                                    </span>
                                  )}
                                  <div
                            ...
                            >
                              ...
                              <span className="hidden sm:inline font-black text-sm whitespace-nowrap">
                                分析
                              </span>
                            </button>
                          </div>
                  ...
                  {!liveResultsLoading &&
                    displayPastCount < liveResults.length && (
                      <div className="flex flex-col items-center justify-center mt-6 p-4 border-[3px] border-black rounded-xl bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full max-w-[400px] mx-auto">
                        <div className="flex items-center justify-between w-full mb-3">
                          ...
                          {sliderPastCount !== displayPastCount && (
                            <Button
                              onClick={() =>
                                setDisplayPastCount(sliderPastCount)
                              }
                  ...
                  {!liveResultsLoading &&
                    displayPastCount >= Math.min(liveResults.length, 50) && (
                      <div className="text-center mt-4 text-zinc-500 font-bold text-sm">
                        已展示全部可用期數
                      </div>
                    )}
      ...
      <Dialog
        open={analysisDrawIndex !== null}
        onOpenChange={(open) => !open && setAnalysisDrawIndex(null)}
      >
          ...
          {analysisDrawIndex !== null &&
            (() => {
              ...
              const singleSizeDist = [
                {
                  label: "1-9",
                  count: singleSixNumbers.filter((n) => n < 10).length,
                },
                {
                  label: "10-19",
                  count: singleSixNumbers.filter((n) => n >= 10 && n <= 19)
                    .length,
                },
                {
                  label: "20-29",
                  count: singleSixNumbers.filter((n) => n >= 20 && n <= 29)
                    .length,
                },
                {
                  label: "30-39",
                  count: singleSixNumbers.filter((n) => n >= 30 && n <= 39)
                    .length,
                },
                {
                  label: "40-49",
                  count: singleSixNumbers.filter((n) => n >= 40 && n <= 49)
                    .length,
                },
              ];
              const singleOddCount = singleSixNumbers.filter(
                (n) => n % 2 !== 0,
              ).length;
              const singleEvenCount = singleSixNumbers.filter(
                (n) => n % 2 === 0,
              ).length;
              const singleRedCount = singleSixNumbers.filter(
                (n) => getBallColor(n) === "red",
              ).length;
              const singleBlueCount = singleSixNumbers.filter(
                (n) => getBallColor(n) === "blue",
              ).length;
              const singleGreenCount = singleSixNumbers.filter(
                (n) => getBallColor(n) === "green",
              ).length;
              ...
              const recentDraws = liveResults.slice(
                startHistoryIndex,
                endHistoryIndex,
              );
              ...
              const endDisplayNumber =
                startDisplayNumber + recentDraws.length - 1;

              // Get all normal numbers from recent draws (excluding the 7th special number)
              const recentNormalNumbers = recentDraws.flatMap((d) =>
                getRawDrawNumbers(d).slice(0, 6),
              );

              const aggSizeDist = [
                {
                  label: "1-9",
                  count: recentNormalNumbers.filter((n) => n < 10).length,
                },
                {
                  label: "10-19",
                  count: recentNormalNumbers.filter((n) => n >= 10 && n <= 19)
                    .length,
                },
                {
                  label: "20-29",
                  count: recentNormalNumbers.filter((n) => n >= 20 && n <= 29)
                    .length,
                },
                {
                  label: "30-39",
                  count: recentNormalNumbers.filter((n) => n >= 30 && n <= 39)
                    .length,
                },
                {
                  label: "40-49",
                  count: recentNormalNumbers.filter((n) => n >= 40 && n <= 49)
                    .length,
                },
              ];
              const aggOddCount = recentNormalNumbers.filter(
                (n) => n % 2 !== 0,
              ).length;
              const aggEvenCount = recentNormalNumbers.filter(
                (n) => n % 2 === 0,
              ).length;
              const aggRedCount = recentNormalNumbers.filter(
                (n) => getBallColor(n) === "red",
              ).length;
              const aggBlueCount = recentNormalNumbers.filter(
                (n) => getBallColor(n) === "blue",
              ).length;
              const aggGreenCount = recentNormalNumbers.filter(
                (n) => getBallColor(n) === "green",
              ).length;
              ...
              return (
                <>
                  ...
                  <div
                    id="analysis-scroll-area"
                    className="p-3 sm:p-4 pb-8 sm:pb-8 flex-1 flex flex-col gap-3 overflow-y-auto"
                  >
                    {/* Current Draw Balls */}
                    <div className="flex shrink-0 gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap justify-center items-center bg-zinc-50 border-[3px] border-black rounded-xl p-2.5 sm:p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] mt-2 mb-1">
                      {drawNumbers.map((num, i) => {
                        ...
                        return (
                          ...
                          >
                            {isSpecial && (
                              <span className="font-black text-xl sm:text-2xl text-zinc-400">
                                +
                              </span>
                            )}
                            <div
                    ...
                    {!Array.isArray(drawObj) &&
                      drawObj.firstPrize !== undefined &&
                      drawObj.firstPrize > 0 && (
                        <div className="bg-[#fffbeb] border-[3px] border-black rounded-xl p-2.5 sm:p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex flex-row items-center justify-between mb-1 mt-1">
                          <span className="font-normal text-zinc-700 text-sm sm:text-base">
                            當期預計頭獎金額
                          </span>
                    ...
                    <div className="flex flex-col shrink-0 gap-2 sm:gap-2 mb-1">
                      <h4 className="font-black text-xl sm:text-lg border-b-[3px] border-black pb-1 text-black">
                        當期頭六碼分佈
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <div className="bg-zinc-50 border-[3px] border-black rounded-xl p-2 sm:p-2 sm:px-3 flex flex-col gap-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                          ...
                          <div className="flex gap-1.5 sm:gap-1.5 text-sm sm:text-sm font-normal flex-wrap">
                            {singleSizeDist.map((dist) => (
                              <div
                                key={dist.label}
                                className="flex bg-white border-2 border-black rounded px-1.5 py-0.5 shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]"
                              >
                                <span className="text-zinc-600 mr-1 sm:mr-0.5">
                                  {dist.label}:
                                </span>
                                <span
                                  className={
                        ...
                        <div className="bg-zinc-50 border-[3px] border-black rounded-xl p-2 sm:p-2 sm:px-3 flex flex-col gap-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] justify-center">
                          ...
                          <span className="text-xl sm:text-lg font-normal">
                            {singleOddCount} 單 : {singleEvenCount} 雙
                          </span>
                        </div>
                        <div className="bg-zinc-50 border-[3px] border-black rounded-xl p-2 sm:p-2 sm:px-3 flex flex-col gap-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] justify-center">
                          <span className="text-sm sm:text-xs font-normal text-zinc-500 uppercase">
                    ...
                    <div className="flex flex-col shrink-0 gap-2 sm:gap-2 mt-1 border-t-[3px] border-black border-dashed pt-3 sm:pt-4">
                      <h4 className="font-black text-xl sm:text-lg border-b-[3px] border-black pb-1 mb-0.5 text-[#3b82f6]">
                        歷史趨勢追蹤
                      </h4>
                      ...
                      <div className="flex items-center gap-2 bg-zinc-50 border-[3px] border-black py-1 px-2 rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                        <Label className="font-normal text-sm sm:text-sm whitespace-nowrap">
                          參考期數
                        </Label>
                        <Slider
                          value={[
                            Math.min(
                              ...
                              Math.max(
                                0,
                                liveResults.length - (analysisDrawIndex + 1),
                              ),
                            ),
                          ]}
                          min={1}
                          ...
                          disabled={
                            liveResults.length - (analysisDrawIndex + 1) === 0
                          }
                        />
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <div className="bg-zinc-50 border-[3px] border-black rounded-xl p-2 sm:p-2 sm:px-3 flex flex-col gap-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                          <span className="text-sm sm:text-xs font-normal text-zinc-500 uppercase">
                            {recentDraws.length > 0
                              ? `第 ${startDisplayNumber} - ${endDisplayNumber} 期 大小分佈`
                              : "暫無足夠歷史數據"}
                          </span>
                          <div className="flex gap-1.5 sm:gap-1.5 text-sm sm:text-sm font-normal flex-wrap">
                        ...
                        <div className="bg-zinc-50 border-[3px] border-black rounded-xl p-2 sm:p-2 sm:px-3 flex flex-col gap-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] justify-center">
                          <span className="text-sm sm:text-xs font-normal text-zinc-500 uppercase">
                            {recentDraws.length > 0
                              ? `第 ${startDisplayNumber} - ${endDisplayNumber} 期 單雙分佈`
                              : "暫無足夠數據"}
                          </span>
                          <span className="text-xl sm:text-lg font-normal">
                            {aggOddCount} 單 : {aggEvenCount} 雙
                          </span>
                        </div>
                        <div className="bg-zinc-50 border-[3px] border-black rounded-xl p-2 sm:p-2 sm:px-3 flex flex-col gap-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] justify-center">
                          <span className="text-sm sm:text-xs font-normal text-zinc-500 uppercase">
                            {recentDraws.length > 0
                              ? `第 ${startDisplayNumber} - ${endDisplayNumber} 期 波色分佈`
                              : "暫無足夠數據"}
                          </span>
                          <span className="text-xl sm:text-lg font-normal flex gap-2">
                            <span className="text-[#FF5C00]">
                    ...
                    <div className="space-y-1.5 shrink-0 sm:space-y-2 mt-2">
                      <h4 className="font-black text-xl sm:text-lg border-b-[3px] border-black pb-1 mb-1">
                        於第 {startDisplayNumber} - {endDisplayNumber}{" "}
                        期內的歷史出現頻率
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-2">
                        {drawNumbers.map((num, i) => {
                          ...
                          recentDraws.forEach((pastDraw, pastIndex) => {
                            if (getRawDrawNumbers(pastDraw).includes(num))
                              appearedIn.push(startDisplayNumber + pastIndex);
                          });
                          ...
                          return (
                            <div
                              key={i}
                              className="flex gap-2 sm:gap-2.5 items-center p-2 sm:p-2 bg-zinc-50 border-[3px] border-black rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                            >
                              <div
                                className={`w-10 h-10 sm:w-11 sm:h-11 shrink-0 rounded-full border-[2px] sm:border-[3px] border-black flex items-center justify-center font-black text-xl sm:text-xl text-black ${bgColor} ${isSpecial ? "border-dashed opacity-90 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" : "shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"}`}
                              >
                                {num}
                              </div>
                              <div className="flex flex-col gap-0 w-full overflow-hidden">
                                <span className="font-normal text-lg sm:text-base leading-tight">
                                  {isSpecial && "特碼 "}共{" "}
                                  <span className="text-[#3b82f6] text-xl sm:text-xl leading-none font-normal">
                                    {appearedIn.length}
                                  </span>{" "}
                                  次
                                </span>
                                <span
                                  className="text-sm sm:text-xs font-normal text-zinc-600 leading-[1.1] sm:leading-tight truncate w-full"
                                  title={
                                    appearedIn.length > 0
                                      ? `(見於: 第 ${appearedIn.join(", ")} 期)`
                                      : "無歷史出現紀錄"
                                  }
                                >
                                  {appearedIn.length > 0
                    ...
                    {(() => {
                      ...
                      const winningNums = getRawDrawNumbers(currentDraw).slice(
                        0,
                        6,
                      );
                      ...
                      const getFreqStats = (
                        drawsCount: number,
                        hotThreshold: number,
                        warmThreshold: number = 1,
                      ) => {
                        const pastDraws = liveResults.slice(
                          analysisDrawIndex + 1,
                          analysisDrawIndex + 1 + drawsCount,
                        );
                        const allPastNums = pastDraws.flatMap((d) =>
                          getRawDrawNumbers(d),
                        );
                        const freqMap = new Map<number, number>();
                        allPastNums.forEach((n) =>
                          freqMap.set(n, (freqMap.get(n) || 0) + 1),
                        );
                      ...
                      const oddCount = winningNums.filter(
                        (n) => n % 2 !== 0,
                      ).length;
                      const evenCount = 6 - oddCount;

                      const rCount = winningNums.filter(
                        (n) => getBallColor(n) === "red",
                      ).length;
                      const bCount = winningNums.filter(
                        (n) => getBallColor(n) === "blue",
                      ).length;
                      const gCount = winningNums.filter(
                        (n) => getBallColor(n) === "green",
                      ).length;
                      ...
                      const dominantColors = colorResults.filter(
                        (c) => c.count > 0,
                      );
                      const colorStr = dominantColors
                        .map((c) => `${c.color} ${c.count} 個`)
                        .join("，");
                      ...
                      const commonTails = Object.entries(tailCounts)
                        .filter(([_, count]) => count > 1)
                        .map(([tail, count]) => `${tail}尾(${count}個)`);
                      ...
                      const emptyZones = zones
                        .map((count, index) =>
                          count === 0
                            ? `${index === 0 ? 1 : index * 10}-${index === 4 ? 49 : index * 10 + 9}`
                            : null,
                        )
                        .filter(Boolean);
                          ...
                          <div className="space-y-4 text-sm sm:text-[15px] font-normal text-zinc-800 relative z-10">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              <div className="flex bg-white border-2 border-black rounded-lg p-3 items-center gap-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.1)]">
                                <div className="bg-green-100 border-2 border-black rounded-full px-3 py-1 font-normal text-xs shrink-0 text-green-800">
                                  分佈範圍
                                </div>
                                <div>
                                  {minNum} 至 {maxNum}
                            ...
                            <div className="flex bg-white border-2 border-black rounded-lg p-3 flex-col items-start gap-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.1)]">
                              <div className="flex items-center gap-3 w-full">
                                <div className="bg-orange-100 border-2 border-black rounded-full px-3 py-1 font-normal text-xs shrink-0 text-orange-800">
                                  特殊形態
                                </div>
                                <div className="flex-1 flex flex-wrap gap-2">
                                  {consecutives > 0 ? (
                                    <span className="bg-red-100 border border-red-300 text-red-800 px-2 py-0.5 rounded text-xs">
                                      出現 {consecutives} 組連號
                                    </span>
                                  ) : (
                                    <span className="bg-zinc-100 border border-zinc-300 text-zinc-600 px-2 py-0.5 rounded text-xs">
                                      無連號
                                    </span>
                                  )}
                                  {commonTails.length > 0 ? (
                                    <span className="bg-purple-100 border border-purple-300 text-purple-800 px-2 py-0.5 rounded text-xs">
                                      同尾: {commonTails.join(", ")}
                                    </span>
                                  ) : (
                                    <span className="bg-zinc-100 border border-zinc-300 text-zinc-600 px-2 py-0.5 rounded text-xs">
                                  ...
                                  {emptyZones.length > 0 && (
                                    <span className="bg-blue-100 border border-blue-300 text-blue-800 px-2 py-0.5 rounded text-xs">
                                      斷區: {emptyZones.join(", ")}
                                    </span>
                                  )}
                                </div>
                            ...
                            <div className="flex bg-white border-2 border-black rounded-lg p-3 flex-col items-start gap-1.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.1)]">
                              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1.5 sm:gap-3 w-full">
                                <div className="bg-yellow-200 border-2 border-black rounded-full px-2.5 py-0.5 font-normal text-[11px] shrink-0 text-yellow-900 whitespace-nowrap text-center mt-0.5 sm:mb-auto">
                                  近期冷熱
                                </div>
                                <div className="flex flex-col gap-2 w-full text-[13px]">
                                  <div className="flex flex-col w-full bg-zinc-50 px-2 py-1.5 rounded border border-black max-w-full">
                                    <div className="flex items-center gap-1.5 w-full">
                                      <span className="font-bold text-zinc-600 min-w-[40px]">
                                        近 5 期
                                      </span>
                                    ...
                                    <div className="flex flex-wrap gap-2 text-[11px] mt-1 pr-2 pt-0.5 border-t border-zinc-200">
                                      {stats5.hot > 0 && (
                                        <span className="text-red-600">
                                          熱: {stats5.hotNums.join(", ")}
                                        </span>
                                      )}
                                      {stats5.warm > 0 && (
                                        <span className="text-orange-600">
                                          溫: {stats5.warmNums.join(", ")}
                                        </span>
                                      )}
                                      {stats5.cold > 0 && (
                                        <span className="text-blue-600">
                                          冷: {stats5.coldNums.join(", ")}
                                        </span>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex flex-col w-full bg-zinc-50 px-2 py-1.5 rounded border border-black max-w-full">
                                    <div className="flex items-center gap-1.5 w-full">
                                      <span className="font-bold text-zinc-600 min-w-[44px]">
                                        近 10 期
                                      </span>
                                      <div className="flex gap-2 font-mono ml-auto">
                                        <span className="w-[32px] text-right">
                                    ...
                                    <div className="flex flex-wrap gap-2 text-[11px] mt-1 pr-2 pt-0.5 border-t border-zinc-200">
                                      {stats10.hot > 0 && (
                                        <span className="text-red-600">
                                          熱: {stats10.hotNums.join(", ")}
                                        </span>
                                      )}
                                      {stats10.warm > 0 && (
                                        <span className="text-orange-600">
                                          溫: {stats10.warmNums.join(", ")}
                                        </span>
                                      )}
                                      {stats10.cold > 0 && (
                                        <span className="text-blue-600">
                                          冷: {stats10.coldNums.join(", ")}
                                        </span>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex flex-col w-full bg-zinc-50 px-2 py-1.5 rounded border border-black max-w-full">
                                    ...
                                    <div className="flex flex-wrap gap-2 text-[11px] mt-1 pr-2 pt-0.5 border-t border-zinc-200">
                                      {stats20.hot > 0 && (
                                        <span className="text-red-600">
                                          熱: {stats20.hotNums.join(", ")}
                                        </span>
                                      )}
                                      {stats20.warm > 0 && (
                                        <span className="text-orange-600">
                                          溫: {stats20.warmNums.join(", ")}
                                        </span>
                                      )}
                                      {stats20.cold > 0 && (
                                        <span className="text-blue-600">
                                          冷: {stats20.coldNums.join(", ")}
                                        </span>
                                      )}
                                    </div>
                                  </div>
                            ...
                            <div className="bg-zinc-900 text-white p-4 rounded-xl border-[3px] border-black mt-2 shadow-[4px_4px_0px_0px_#FFE867]">
                              ...
                              <ul className="list-disc pl-5 space-y-1.5 text-[14px] text-zinc-200">
                                <li>
                                  ...
                                  ，號碼分佈
                                  {consecutives > 0 ? "偏向聚集" : "較為分散"}。
                                </li>
                                {emptyZones.length > 0 && (
                                  <li>
                                    出現明顯「斷區」，未來若針對這類趨勢，可善用「自訂號碼分析範圍」排除{" "}
                                    <span className="font-normal">
                                      {emptyZones.join("、")}
                                    </span>
                                    。
                                ...
                                {commonTails.length > 0 && (
                                  <li>
                                    同尾數效應 ({commonTails.join(", ")})
                                    發生，適時挑選尾數靈感有助提升機率。
                                  </li>
                                )}
                                <li>
                                  <span className="font-normal">
                                    {recentStrategySuggestion}
                                  </span>
                                </li>
                              </ul>
                            </div>
        ...
        <DialogContent className="border-4 border-black rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] sm:max-w-[500px] w-[95vw] bg-white text-black p-0 overflow-hidden text-center top-1/2 -translate-y-1/2 flex flex-col max-h-[85vh]">
          ...
          <div className="p-5 text-sm font-bold text-zinc-700 text-left leading-relaxed overflow-y-auto min-h-0 flex-1 space-y-4 custom-scrollbar">
            <div className="bg-[#f0fdf4] p-3 rounded-lg border-2 border-green-200">
              <h4 className="text-green-700 font-extrabold mb-1 flex items-center gap-1.5">
                <span className="text-lg">🌡️</span> 溫度過濾策略
                (每次生成隨機決策)
              </h4>
              <ul className="list-disc pl-5 mt-1 space-y-1 text-zinc-800 font-medium">
                <li>
                  ...
                  排除近 1 期的大熱號碼，保留第 2 - 5 期的微熱號碼。
                </li>
                <li>
                  <strong className="font-bold text-green-800">
                    乘勝追擊：
                  </strong>
                  不排除近期，反而鎖定近 1 - 3 期頻繁出現的當炒大熱號碼。
                </li>
                <li>
                  <strong className="font-bold text-green-800">
                    逆向博冷：
                  ...
                  完全排除近 6
                  期出現過的大熱號碼，專挑久未出現的「冷門」號碼博取高回報！
                </li>
              </ul>
            </div>

            <div className="bg-[#fdf4ff] p-3 rounded-lg border-2 border-fuchsia-200">
              <h4 className="text-fuchsia-700 font-extrabold mb-1 flex items-center gap-1.5">
                <span className="text-lg">🎨</span> 顏色波色決策
              </h4>
              <ul className="list-disc pl-5 mt-1 space-y-1 text-zinc-800 font-medium">
                <li>
                  大多數情況下會維持
                  <strong className="font-bold">紅白藍三色均勻</strong>分配。
                </li>
                <li>
                  偶爾會根據近期最常開出的兩種波色，
                  <strong className="font-bold">鎖定那兩種波色</strong>
                  ，並按比例投放資源（例如只買旺開的紅藍波）。
                </li>
              </ul>
            </div>
            ...
            <div className="bg-[#fefce8] p-3 rounded-lg border-2 border-yellow-200">
              <h4 className="text-yellow-700 font-extrabold mb-1 flex items-center gap-1.5">
                <span className="text-lg">📊</span> 大數據注數分配
              </h4>
              <p className="text-zinc-800 font-medium">
                在使用「2合 / 3合組合」策略的資源不再固定是 20%，而是會動態在{" "}
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="bg-[#ffedd5] p-3 rounded-lg border-2 border-orange-200 flex-1">
                <h4 className="text-orange-600 font-extrabold mb-1 flex items-center gap-1.5">
                  <span className="text-lg">🎲</span> 採用「2合」策略
                </h4>
                <p className="text-zinc-800 font-medium">
                  系統會從初步生成的號碼中，隨機抽取 1 至 3
                  個號碼作為「基數」。分析過去期數數據，找出與該基數同開機率最高的頭
                  6 個伴隨號碼 (2合組合)，加入高機率號碼並隨機替換原號碼。
                </p>
              </div>
              <div className="bg-[#e0f2fe] p-3 rounded-lg border-2 border-blue-200 flex-1">
                <h4 className="text-blue-600 font-extrabold mb-1 flex items-center gap-1.5">
                  <span className="text-lg">🎯</span> 採用「3合」策略
                </h4>
                <p className="text-zinc-800 font-medium">
                  系統會從初步生成的號碼中，隨機抽取 1 至 3
                  對號碼作為「基數組合」。找出與該兩碼同開機率最高的頭 6
                  個伴隨號碼 (3合組合)，加入高機率號碼並隨機替換原號碼。
                </p>
              </div>
            </div>
            ...
            <p className="text-[12px] text-zinc-500 bg-zinc-50 p-2 rounded border-2 border-zinc-200 mt-2 font-medium">
              * 系統在隨機剔走 /
              替換號碼時，會自動避開基數、新加入的高機率號碼及您指定的必出幸運號碼。
            </p>
          </div>
      ...
      <Dialog
        open={!!viewingBetExpl}
        onOpenChange={(open) => {
          if (!open) setViewingBetExpl(null);
        }}
      >
          ...
          <DialogHeader className="bg-[#BAE6FD] border-b-4 border-black p-4 sm:p-6 pb-4">
            <DialogTitle className="text-xl sm:text-2xl font-black text-black flex items-center justify-between gap-2 max-w-full">
              <span className="flex items-center gap-2 pr-4 min-w-0">
                ...
                <span className="truncate">
                  第{" "}
                  {viewingBetExpl?.index !== undefined
                    ? viewingBetExpl.index + 1
                    : ""}{" "}
                  注：大數據選號說明
                </span>
              </span>
          ...
          <div className="p-4 sm:p-6 bg-zinc-50 shrink-0 min-h-[100px] max-h-[60vh] sm:max-h-[85vh] overflow-y-auto">
            <div className="flex flex-col gap-2 sm:gap-3">
              {viewingBetExpl?.bet?.explanations?.map((expl, idx) => (
                <div
                  key={idx}
                  className="bg-white border-2 border-black p-3 rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-start gap-2 sm:gap-3 break-words relative overflow-hidden"
                >
              ...
              {(!viewingBetExpl?.bet?.explanations ||
                viewingBetExpl.bet.explanations.length === 0) && (
                <div className="text-center font-bold text-zinc-500 py-4">
                  隨機生成注數，無特定大數據說明。
                </div>
              )}
            </div>
              ...
              {(() => {
                ...
                const renderBall = (n: number) => {
                  ...
                  return (
                    <div
                      key={n}
                      className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full border-[2.5px] border-black flex items-center justify-center font-black text-base sm:text-lg text-black ${bgColor} shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)]`}
                    >
                      {n}
                    </div>
                  );
                  ...
                  return (
                    <div className="flex flex-col gap-2 items-center w-full my-2">
                      <div className="relative">
                        ...
                        <div className="absolute -bottom-3 -right-2 flex items-center justify-center w-[24px] h-[24px] sm:w-[28px] sm:h-[28px] rounded-full bg-black text-[#FFE867] font-black text-[11px] sm:text-[13px] border-[2px] border-white shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] z-10 transform rotate-6">
                          拖
                        </div>
                      </div>
                      <div
      ...
      <div className="fixed top-0 left-0 -z-50 pointer-events-none opacity-0 overflow-hidden w-0 h-0">
        <div
          id="capture-area"
          className={`bg-[#1e1e1e] font-sans flex flex-col p-8 items-center ${generatedBets.length >= 13 ? "w-[1450px]" : generatedBets.length >= 11 ? "w-[1000px]" : isAiGenerated && aiReasoning.length > 0 ? "w-[650px]" : "w-[500px]"}`}
        >
          <h1 className="text-[40px] font-black tracking-widest mb-6 text-[#FFE867] leading-none">
            您的幸運號碼
          </h1>
          <div
            className={`w-full ${generatedBets.length >= 13 ? "grid grid-cols-3 gap-x-8 gap-y-7 justify-items-center" : generatedBets.length >= 11 ? "grid grid-cols-2 gap-x-8 gap-y-7 justify-items-center" : "flex flex-col gap-7 items-center"}`}
          >
            {generatedBets.map((bet, index) => (
              <div
                key={index}
              ...
              >
                <div
                  className={`flex flex-col sm:flex-row items-center sm:items-stretch sm:justify-start w-full mx-auto bg-white border-[4px] border-black rounded-3xl min-h-[70px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] box-border px-4 py-3 relative overflow-visible h-auto max-w-[900px] ${!bet.isBankerLegs && "sm:w-fit"}`}
                >
                  <div className="flex items-center gap-3 w-full">
                          ...
                          return (
                            <div className="flex flex-col gap-2 items-center w-full max-w-[800px] my-3">
                              <div className="relative">
                                ...
                                <div className="absolute -bottom-3 -right-2 flex items-center justify-center w-[26px] h-[26px] sm:w-[32px] sm:h-[32px] rounded-full bg-black text-[#FFE867] font-black text-[12px] sm:text-[14px] border-[2px] border-white shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] z-10 transform rotate-6">
                                  拖
                                </div>
                              </div>
                              <div
                              ...
                              >
                                {bet.numbers
                                  .slice(bet.bankersCount)
                                  .map((num, i) =>
                                    renderBall(num, i + bet.bankersCount!),
                                  )}
                              </div>
                            </div>
                          );
          ...
          <div className="mt-8 bg-[#ffedd5] border-[4px] border-black rounded-2xl p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full max-w-2xl text-center">
            <h3 className="font-black text-xl mb-4 flex items-center justify-center gap-2 min-w-0">
            ...
            {isAiGenerated ? (
              <div className="w-full text-left bg-[#bbf7d0] border-[3px] border-[#16a34a] rounded-lg p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] mt-2 mb-3">
                <div className="font-black text-[#166534] mb-3 flex items-center gap-1.5 text-lg">
                  <Sparkles className="w-5 h-5 shrink-0" />
                  AI 大數據智能選號 (綜合近期 {aiAnalysisDrawsUsed} 期) -
                  分析筆記：
                </div>
                ...
                <ul className="list-disc pl-6 space-y-2 text-base font-bold text-zinc-700">
                  ...
                  {enableRecent && (
                    <li>
                      近期期數過濾:{" "}
                      {recentMode === "include"
                        ? `只買近 ${recentCount} 期內出現過的號碼`
                        : `排除近 ${recentCount} 期內出現過的號碼`}
                    </li>
                  )}
                  {enableExcludeUnseen && (
                    <li>
                      近期期數過濾: 包含近期出現過，但排除近{" "}
                      {excludeUnseenCount} 期未出現的號碼{" "}
                      {excludeUnseenIncludeSpecial
                        ? "(包含特碼)"
                        : "(不含特碼)"}
                    </li>
          ...
          <div className="mt-8 mb-4 p-4 bg-white rounded-2xl border-[4px] border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center">
            <QRCodeSVG
              ...
              size={160}
            />
            <div className="mt-3 text-sm font-black text-black">
              快速對獎・SCAN ME
            </div>
          </div>
          ...
          <div className="p-4 sm:p-5 flex-1 overflow-y-auto space-y-6 max-h-[calc(90vh-80px)] custom-scrollbar">
            <div className="space-y-4 bg-white border-[3px] border-black p-4 rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
              ...
              <div className="pt-4 border-t-2 border-black border-dashed flex flex-col gap-4">
                ...
                >
                  <div
                    className={`w-6 h-6 rounded flex items-center justify-center border-2 border-black shrink-0 transition-colors ${aiBankerMode ? "bg-black text-[#FFE867]" : "bg-white"}`}
                  >
                    {aiBankerMode && (
                  ...
                  <div>
                    <div className="font-black text-sm sm:text-[15px] text-zinc-900">
                      自動生成拖膽策略 (節省成本)
                    </div>
                    <div className="font-normal text-[11px] sm:text-xs text-zinc-600 leading-tight">
                      使用大數據智能「膽拖」配搭，分散投注並提高號碼覆蓋率。
                    </div>
                  </div>
                </div>
                ...
                {!aiBankerMode ? (
                  ...
                  <div className="space-y-4 pt-2 bg-[#ffedd5]/30 p-3 sm:p-4 rounded-xl border-2 border-black">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <Label className="font-black text-xs sm:text-sm text-zinc-800">
                          生成拖膽注數
                        </Label>
      ...
      <Dialog open={isCheckDialogOpen} onOpenChange={setIsCheckDialogOpen}>
        <DialogContent className="w-[95vw] max-w-[95vw] sm:max-w-3xl bg-[#fff7ed] border-[4px] border-black rounded-[24px] p-0 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col max-h-[90vh] overflow-hidden top-[5vh] translate-y-0 sm:top-1/2 sm:-translate-y-1/2">
          ...
          <div className="p-4 sm:p-5 flex-1 overflow-y-auto space-y-4 min-h-0 custom-scrollbar">
            <div className="space-y-2">
              <Label className="font-black text-base sm:text-lg">
                選擇您想核對的開彩期數：
              </Label>
              <Select
                value={checkDrawIndex.toString()}
                onValueChange={(val) => {
              ...
              >
                <SelectTrigger className="w-full bg-white border-2 border-black rounded-xl font-bold min-h-[44px] h-auto py-2 text-base shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-left whitespace-normal">
                  <div className="flex-1 text-left sm:flex sm:items-center sm:gap-2">
                    {(() => {
                      if (!liveResults || !liveResults[checkDrawIndex])
                        return "選擇期數...";
                      ...
                      return (
                        <>
                          <div className="text-sm sm:text-base leading-tight">
                            {checkDrawIndex === 0
                              ? `最近一期${formattedDate}`
                              : `前 ${checkDrawIndex} 期${formattedDate}`}{" "}
                            :
                          </div>
                  ...
                  {liveResults.map((draw, i) => {
                    ...
                    return (
                      <SelectItem
                        key={i}
                        value={i.toString()}
                        className="font-bold cursor-pointer hover:bg-neutral-100 p-2 sm:p-3 items-start flex-col focus:bg-[#FFE867]"
                      >
                        <div className="flex flex-col sm:flex-row w-full sm:items-center sm:gap-2 text-left whitespace-normal">
                          ...
                          <span className="text-base sm:text-lg tracking-wider whitespace-normal">
                            {numbers.slice(0, 6).join(",")} + ({numbers[6]})
                          </span>
                        </div>
                      </SelectItem>
            ...
            {!checkResults ? (
              ...
              >
                <TabsList className="grid grid-cols-2 gap-2 bg-transparent h-auto w-full mb-2">
                  <TabsTrigger
                    value="upload"
                    className="w-full font-black text-lg sm:text-xl py-0.5 px-0.5 text-black bg-orange-100 border-[3px] border-black data-[state=active]:bg-[#FFD700] data-[state=active]:border-[3px] data-[state=active]:border-black data-[state=active]:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-[14px] transition-all whitespace-nowrap h-auto min-h-[40px]"
                  >
                    上傳截圖
                  </TabsTrigger>
                  <TabsTrigger
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col items-center justify-center p-4 sm:p-6 border-[4px] border-dashed border-[#FF4D4D] rounded-2xl bg-[#FFE867] text-center hover:bg-[#FFD700] hover:border-black cursor-pointer transition-all relative overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] group">
                      ...
                      <div className="mt-2 text-[12px] sm:text-[13px] font-black bg-white text-black px-2 py-1 border-[3px] border-black rounded-lg transform -rotate-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] inline-block">
                        ⚠️ 必需由
                        <span className="text-[#3b82f6]">本系統下載</span>
                      </div>
                    </div>
                  </div>
                </TabsContent>
                ...
                <div className="space-y-3 hide-scrollbar">
                  {checkResults.map((res, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`p-4 border-[3px] ${borderPrize} rounded-xl font-bold ${bgPrize} ${isWin ? "shadow-[4px_4px_0px_0px_#FFE867]" : "shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"}`}
                      >
                            ...
                            <div className="flex gap-1.5 sm:gap-2">
                              <span className="bg-white border-[2.5px] border-zinc-500 px-2 py-0.5 rounded-md shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,0.5)] text-zinc-600 text-sm sm:text-base font-bold whitespace-nowrap">
                                ...
                                <strong
                                  className={
                                    matchCount > 0
                                      ? "text-[#3b82f6] text-lg sm:text-xl"
                                      : "text-zinc-600"
                                  }
                                >
                                  {matchCount}
                        ...
                        <div className="flex flex-wrap gap-1 sm:gap-2 pt-1 pb-1 px-0.5 sm:px-1 w-full justify-start max-w-full">
                          {res.bet.map((num: number, idx: number) => {
                            ...
                            const currentCheckDraw =
                              liveResults[checkDrawIndex];
                            const currentWinningNumbers = currentCheckDraw
                              ? getRawDrawNumbers(currentCheckDraw)
                              : [];
                            const isMatchSpecial =
                              currentWinningNumbers.length > 6 &&
                              num === currentWinningNumbers[6];
        ...
        <DialogContent className="w-[95vw] max-w-[95vw] sm:max-w-4xl bg-white border-[4px] border-black rounded-[24px] p-0 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col max-h-[90vh] overflow-hidden top-[5vh] translate-y-0 sm:top-1/2 sm:-translate-y-1/2">
          ...
          <div className="p-4 sm:p-5 flex-1 overflow-y-auto min-h-0 custom-scrollbar flex flex-col gap-4">
            <div className="text-sm sm:text-base font-bold text-zinc-700 bg-blue-50 border-2 border-blue-200 p-3 rounded-xl">
              💡 <strong>預計頭獎與總和分析：</strong> AI
              統計各個預計頭獎區間的歷史中獎數據，發現頭獎金額往往與總和及特定號碼規律（如連號）有關。高預計頭獎的號碼分佈往往較偏鋒，可作為選號的重要指標。
            </div>
            ...
            <div className="flex flex-col gap-2 bg-white border-[3px] border-black p-3 sm:p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              ...
              <span className="text-xs text-zinc-500 font-bold self-end">
                預設分析 50 期
              </span>
            </div>
              ...
              return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {brackets.map((b, idx) => {
                    ...
                    const consPct = hasItems
                      ? Math.round(
                          (b.items.filter((i) => i.hasConsecutive).length /
                            b.items.length) *
                            100,
                        )
                      : "-";
                    ...
                    const oddPct = hasItems
                      ? Math.round((oddCount / totalNums) * 100)
                      : "-";
                    const evenPct = hasItems
                      ? Math.round((evenCount / totalNums) * 100)
                      : "-";

                    const rcPct = hasItems
                      ? Math.round((rCount / totalNums) * 100)
                      : "-";
                    const bcPct = hasItems
                      ? Math.round((bColorCount / totalNums) * 100)
                      : "-";
                    const gcPct = hasItems
                      ? Math.round((gCount / totalNums) * 100)
                      : "-";

                    const r1Pct = hasItems
                      ? Math.round((range1 / totalNums) * 100)
                      : "-";
                    const r10Pct = hasItems
                      ? Math.round((range10 / totalNums) * 100)
                      : "-";
                    const r20Pct = hasItems
                      ? Math.round((range20 / totalNums) * 100)
                      : "-";
                    const r30Pct = hasItems
                      ? Math.round((range30 / totalNums) * 100)
                      : "-";
                    const r40Pct = hasItems
                      ? Math.round((range40 / totalNums) * 100)
                      : "-";

                    return (
                      <div
                        key={idx}
                        className={`bg-white border-[3px] border-black p-3 sm:p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-2 ${!hasItems ? "opacity-60" : ""}`}
                      >
                        <div className="font-black text-base sm:text-lg text-black border-b-[3px] border-black pb-1 mb-1 flex items-center justify-between">
                          <span>{b.label}</span>
                          <span className="text-zinc-500 text-xs sm:text-sm bg-zinc-100 border border-black px-1.5 py-0.5 rounded-md shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">
                            共 {b.items.length} 期
                          </span>
                        </div>
                            ...
                            {hasItems ? (
                              <div className="grid grid-cols-5 gap-1 text-[10px] sm:text-xs text-center border-t border-yellow-200 pt-1 mt-0.5">
                                <div className="flex flex-col">
                                  <span className="text-zinc-500">1-9</span>
                                  <span className="font-black text-black">
                                    {r1Pct}%
                                  </span>
                                </div>
                                ...
                                <div className="flex flex-col">
                                  <span className="text-zinc-500">20s</span>
                                  <span className="font-black text-black">
                                    {r20Pct}%
                                  </span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-zinc-500">30s</span>
                                  <span className="font-black text-black">
                                    {r30Pct}%
                                  </span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-zinc-500">40s</span>
                                  <span className="font-black text-black">
                                    {r40Pct}%
                                  </span>
                                </div>
                              ...
                              <span className="text-right text-yellow-600 font-black">
                                -
                              </span>
                            )}
                          </div>
            ...
            {(() => {
              const chartData = liveResults
                ...
                .map((d) => {
                  const nums =
                    !Array.isArray(d) && d.numbers ? d.numbers.slice(0, 6) : [];
                  const sum = nums.reduce((a, b) => a + b, 0);
                  const firstPrize =
                    !Array.isArray(d) && d.firstPrize ? d.firstPrize : 0;
                  const payoutInMillions =
                    Math.round((firstPrize / 1000000) * 10) / 10;
                  return {
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-white border-[3px] border-black p-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-lg font-bold">
                                <p className="text-base border-b-2 border-zinc-200 pb-1 mb-1">
                                  {data.date}
                                </p>
                                ...
                                <p className="text-[#FF4D4D]">
                                  預計頭獎:{" "}
                                  {data.payoutOriginal ? (
                                    `$${data.payoutOriginal.toLocaleString()}`
                                  ) : (
              ...
              <table className="w-full text-left font-bold text-sm sm:text-base border-collapse min-w-[500px]">
                <thead className="bg-[#f1f5f9] border-b-[3px] border-black">
                  <tr>
                    ...
                    <th className="p-2 sm:p-3 whitespace-nowrap text-center">
                      號碼
                    </th>
                    <th className="p-2 sm:p-3 whitespace-nowrap text-center">
                      總和
                    </th>
                    <th className="p-2 sm:p-3 whitespace-nowrap text-right">
                      預計頭獎
                    </th>
                  </tr>
                </thead>
                  ...
                  {liveResults
                    ...
                    .map((d, i) => {
                      ...
                      return (
                        <tr
                          key={i}
                          className={`border-b-2 border-black/10 ${isHighPrize ? "bg-orange-50" : ""}`}
                        >
                          <td className="p-2 sm:p-3 whitespace-nowrap">
                            {d.date}
                          </td>
                          <td className="p-2 sm:p-3 text-center tracking-widest">
                            {nums.join(", ")}
                          </td>
                          <td className="p-2 sm:p-3 text-center">
                            <span
                              className={`px-2 py-0.5 rounded border border-black ${sum < 100 ? "bg-blue-200 text-blue-900" : sum > 200 ? "bg-red-200 text-red-900" : "bg-green-200 text-green-900"}`}
                            >
                              {sum}
                            </span>
                          </td>
                          <td className="p-2 sm:p-3 whitespace-nowrap text-right text-[#FF4D4D] font-black">
                            ...
                            <span className="text-xs text-zinc-500">
                              / {d.firstPrizeWinners ?? 0}注
                            </span>
                          </td>
                        </tr>
                      );
                    })}
                  {liveResults.filter((d) => !Array.isArray(d)).length ===
                    0 && (
                    <tr>
            ...
            <div className="flex justify-center mt-2">
              <Button
                ...
                className="border-4 border-black font-black text-black bg-zinc-200 hover:bg-zinc-300 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-xl px-8 h-12 text-lg"
              >
                關閉
              </Button>
            </div>
      ...
      <Toaster
        ...
        toastOptions={{
          classNames: {
            toast:
              "group !bg-white data-[type=success]:!bg-[#FFE867] !border-[3px] !border-black !rounded-2xl !shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] sm:!border-4 sm:!shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-black font-black text-[15px] sm:text-lg px-4 py-3 sm:px-5 sm:py-4 flex gap-3 w-full items-start",
